// POST /api/gameideas — manage the admin "game ideas" list (admin only).
...
export async function onRequestPost(context) {
  ...
  if (action === "add") {
    ...
    if (text) {
      await env.DB.prepare("INSERT INTO game_ideas (text) VALUES (?)").bind(text).run();
    }
  } else if (action === "toggle") {
    const id = Number(form.get("id"));
    if (Number.isInteger(id)) {
      await env.DB.prepare("UPDATE game_ideas SET done = 1 - done WHERE id = ?").bind(id).run();
    }
  } else if (action === "delete") {
    const id = Number(form.get("id"));
    if (Number.isInteger(id)) {
      await env.DB.prepare("DELETE FROM game_ideas WHERE id = ?").bind(id).run();
    }
  }
  return backToAdmin();
}
